...
import { useAquaData, Vessel, useAquaAuth } from '../../components/context/AquaRegCONTEXT';
import { Badge } from '../../components/ui/badge';
import { Dialog, DialogContent } from "../../components/ui/dialog";

export default function InspectionRecords() {
  const { Vessels = [], loading, completeInspection } = useAquaData() as any;
  const { currentUser } = useAquaAuth();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedRecord, setSelectedRecord] = useState<Vessel | null>(null);

  // --- EXAMPLE HANDLER TO COMPLETE AN INSPECTION ---
  const handleQuickComplete = async (vesselId: string) => {
    const orNumber = prompt("Enter Official Receipt (O.R.) Number:");
    if (!orNumber) return;
    
    const remarks = prompt("Enter inspection remarks (optional):") || "Passed standard municipal vessel inspection.";

    try {
      const success = await completeInspection(vesselId, orNumber, remarks);
      if (success) {
        alert("Inspection successfully recorded and archived!");
      } else {
        alert("Failed to update inspection record.");
      }
    } catch (err: any) {
      console.error(err);
