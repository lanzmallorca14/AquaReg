...
const generateCustomId = async (category: string) => {
  let prefix = 'RM';
  if (category === 'payao') prefix = 'PY';
  else if (category === 'pangulong') prefix = 'PG';
  else if (category === 'gears') prefix = 'FG';

  try {
    // Query Supabase for existing IDs matching the current prefix
    ...
    // Extract numbers from existing IDs (e.g., "RM-0004" -> 4)
    const numbers = data
      ?.map(v => {
    ...
    // Find the max number and add 1, default to 1 if empty
    const nextNum = numbers.length > 0 ? Math.max(...numbers) + 1 : 1;

    // Format with leading zeros (e.g., 5 -> "00005")
    return `${prefix}-${String(nextNum).padStart(5, '0')}`;
  } catch (err) {
    console.error('Error generating custom ID:', err);
    // Fallback ID if offline or query fails
    return `${prefix}-00001`;
  }
};
...
export default function AquaRegNewRegistration() {
  ...
  const { Vessels = [], addVessel, isDuplicateName } = useAquaData();
  ...
  useEffect(() => {
    ...
    if (prefillData) {
      ...
      setF({
        assetCategory: prefillData.asset_category || prefillData.type || 'vessel',
        vesselName: (prefillData.vessel_name || prefillData.gear_type || '').toUpperCase(),
        ownerName: (prefillData.owner_name || prefillData.owner || '').toUpperCase(),
        ...
        phone: prefillData.cp_number || prefillData.phone || '+639',
  ...
  const handleRenewalSearch = () => {
    ...
    setF(prev => ({
      ...
      phone: existing.cp_number ?? "+639",
      assetCategory: existing.asset_category === "gears" ? "gears" : existing.asset_category,
  ...
  const up = useCallback((k: string, v: any) => {
    setF(p => {
      let val = v;
      if (k === 'phone') {
        const numbersOnly = String(v).replace(/[^0-9]/g, '');
        const digits = numbersOnly.startsWith('639') ? numbersOnly : '639' + numbersOnly;
        val = '+' + digits.slice(0, 12);
      } 
      else if (k === 'ownerAge') {
        val = String(v).replace(/[^0-9]/g, '').slice(0, 3);
      }
      else if (['ownerName', 'vesselName', 'sitio', 'placeOfBuilt'].includes(k)) {
        val = String(v).replace(/[^a-zA-Z0-9\s,.]/g, '').toUpperCase();
      }
      return { ...p, [k]: val };
    });
  }, []);
 ...
 const errs = useMemo(() => {
    ...
    // 🛑 BLOCK IF DUPLICATE NAME FOUND (Excluding Re-audit of the same record)
    if (isDuplicate && s.mode !== 'RE-AUDIT') {
      r.push("Vessel or asset name already exists in the registry");
    }

    if (hasInvalidFormat) r.push("Vessel name must have a space before the number");
    if (f.ownerName && !f.ownerName.includes(',')) r.push("Format: LAST, FIRST");
    
    const ageNum = parseInt(f.ownerAge);
    if (!f.ownerAge || ageNum < 18) r.push("Invalid Age (18+)");
    if (!f.barangay) r.push("Select Barangay");
    if (f.phone.length < 13) r.push("Phone Incomplete");
...
const send = async () => {
    ...
    try {
      ...
      // --- GENERATE INCREMENTAL ID IF NOT A RE-AUDIT/RENEWAL ---
      const existingTargetId = location.state?.existingId || s.id || JSON.parse(localStorage.getItem('reRegisterVesselData') || '{}').id;
      
      const generatedId = existingTargetId || await generateCustomId(f.assetCategory);

      if (!navigator.onLine) {
        const localId = generatedId;
        const offlineVessel = { id: localId, ...payload, created_at: new Date().toISOString() };
        
        const offlineQueue = JSON.parse(localStorage.getItem('offline_Vessels_queue') || '[]');
        offlineQueue.push({ id: localId, payload });
        localStorage.setItem('offline_Vessels_queue', JSON.stringify(offlineQueue));
        ...
        setS(prev => ({ ...prev, showSuccess: true }));
        return;
      }
      ...
      if (s.id || existingTargetId) {
        res = await query.upsert(recordPayload, { onConflict: 'id' }).select().single();
      } else {
        res = await query.insert(recordPayload).select().single();
      }
      ...
      if (existingTargetId) setCreatedRM(existingTargetId);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 rounded-3xl text-center space-y-6 shadow-2xl border-none bg-white">
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border text-left space-y-2">
            ...
            <div className="flex justify-between text-[10px] font-bold uppercase text-slate-400">
              <span>Owner:</span>
              <span className="text-slate-900">{f.ownerName}</span>
            </div>
            <div className="flex justify-between text-[10px] font-bold uppercase text-slate-400">
              <span>Vessel:</span>
              <span className="text-slate-900">{f.vesselName}</span>
            </div>
      ...
      <main className="max-w-6xl mx-auto space-y-6 pt-4 pb-20">
        ...
        <div className={`grid lg:grid-cols-12 gap-6 transition-opacity ${s.mode === 'RENEWAL' && !s.id ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
          <div className="lg:col-span-8 space-y-6">
            <Card className="p-6 rounded-3xl shadow-lg space-y-4">
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-7 space-y-1">
                  <Label className="text-[8px] font-black uppercase text-slate-400">Owner Name</Label>
                  <Input value={f.ownerName} onChange={e => up('ownerName', e.target.value)} className="h-10 text-sm font-black uppercase" placeholder="LAST, FIRST"/>
                </div>
                ...
                <div className="col-span-3 space-y-1">
                  <Label className="text-[8px] font-black uppercase text-slate-400">CP Number</Label>
                  <Input value={f.phone} onChange={e => up('phone', e.target.value)} className="h-10 text-sm font-mono"/>
                </div>
              </div>
               ...
               <div className="space-y-1">
                  ...
                  {isDuplicate && s.mode !== 'RE-AUDIT' && (
                    <p className="text-[9px] font-black text-amber-600 mt-2 uppercase flex items-center gap-1">
                       <ShieldAlert size={12}/> This name is already recorded in the registry
                    </p>
                  )}
                  {hasInvalidFormat && (
            ...
            <Tabs value={f.assetCategory} onValueChange={v => up('assetCategory', v)}>
              <TabsList className="w-full bg-slate-100 p-1 h-14 flex gap-1 rounded-[1.2rem] mb-4 shadow-inner">
                {['vessel', 'pangulong', 'payao', 'gears'].map(t => (
                  <TabsTrigger key={t} value={t} className="flex-1 text-[9px] font-black uppercase rounded-lg">
                    {t === 'vessel' && <Ship size={14} className="mr-2"/>}
                    {t === 'payao' && <Anchor size={14} className="mr-2"/>}
              ...
              <TabsContent value="gears">
                <Card className="p-6 rounded-3xl bg-slate-50/50">
                    ...
                    <>
                      ...
                      <div className="p-4 bg-slate-50 rounded-2xl grid grid-cols-3 gap-3">
                        {['length', 'width', 'depth'].map(d => (
                          <div key={d} className="space-y-1 text-center border-r border-slate-200 last:border-none">
                            <Label className="text-[7px] uppercase text-slate-400 block">{d} (m)</Label>
                            <Input type="number" step="0.01" value={f[d as keyof typeof f] as string} onChange={e => up(d, e.target.value)} className="h-10 text-center font-black border-none bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0"/>
                          </div>
                        ))}
                      </div>
                    </>
